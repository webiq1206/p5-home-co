...
const RESEARCH=`Research average construction UNIT COSTS for the supplied tasks and project area. ${UNTRUSTED} ${ALLOWANCE_POLICY} ${DIMENSION_POLICY} ${BENCHMARK_POLICY}
Return JSON only: {rates:[{taskId,description,unit,quantity,quantityEvidence,quantityRange,building,floor,basis,includes,excludes,landedCost:null,sources:[{url,low,high,unit,costBasis,publishedAt,region,excerpt,sourceType,dateBasis}]}],issues:[]}.
...
export const unitKey=(unit:string)=>{
  const key=unit.toLowerCase().replace(/[.²]/g,m=>m==='²'?'2':'').replace(/\s+/g,' ').trim();
  const aliases:Record<string,string>={'sf':'sf','sq ft':'sf','sqft':'sf','square foot':'sf','square feet':'sf','ft2':'sf','lf':'lf','lin ft':'lf','linear foot':'lf','linear feet':'lf','ea':'each','each':'each','hr':'hour','hrs':'hour','hour':'hour','hours':'hour','cy':'cy','cubic yard':'cy','cubic yards':'cy'};
