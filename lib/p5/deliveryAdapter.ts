...
export async function sendEmail(input:{to:string;subject:string;text:string;attachments:{filename:string;content:Buffer}[];key:string}){
  ...
  const result=await transport.sendMail({from:process.env.SMTP_FROM||`${brand.name} <${brand.email}>`,to:input.to,replyTo:brand.email,subject:input.subject,text:input.text,attachments:input.attachments,messageId:`<${input.key}@${brand.domain}>`});
