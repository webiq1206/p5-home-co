...
export async function putDraft(request:Request){
  try{
    ...
    const answers=parseAnswers(raw.answers);const existing=await readDraft(id,key);
    ...
    if(raw.reviewed===true){
      ...
      reviewed={text:raw.text,answers,extraction,uploads:existing?.uploads||[],reviewedAt:new Date().toISOString(),
    ...
    const draft=await saveDraft(id,key,ESTIMATOR_BRAND.id,{text:raw.text,answers,extraction,reviewed,contact},raw.revision);
    return json({draft});
