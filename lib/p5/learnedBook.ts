...
export function learnableLines(rules:readonly CostRule[],service:string,draftId:string,known:readonly {description:string;unit:string}[],now=new Date()):LearnedLine[]{
  ...
  for(const rule of rules){
    ...
    if(!(typeof unitCost==='number'&&unitCost>0)||!dimension(rule.unit)||dimension(rule.unit)==='lump')continue;
    ...
    if(resemblesExisting(candidate,[...book,...known,...accepted]))continue;
    const hash=createHash('sha256').update(`${candidate.description.toLowerCase()}|${unitKey(rule.unit)}`).digest('hex').slice(0,10);
    accepted.push({code:`PB-L-${hash}`,description:candidate.description,unit:UNIT_REGISTRY[unitKey(rule.unit)].label,amount:Math.round(unitCost*100)/100,type:TYPE[String(rule.category)]||'Other',service,source:`Learned from estimate ${draftId} (${rule.id}); direct cost`,learnedAt:now.toISOString(),uses:1});
  }
  return accepted;
}
...
export async function saveLearnedLines(rules:readonly CostRule[],service:string,draftId:string):Promise<number>{
  const known=await readLearnedLines();
  const lines=learnableLines(rules,service,draftId,known);
  for(const line of lines)await query("INSERT INTO p5_estimator_policy (id,version,payload,updated_by) VALUES ($1,1,$2,'learned-book') ON CONFLICT (id) DO NOTHING",[`book-learned:${line.code}`,JSON.stringify(line)]);
...
/** Learned lines as catalog rates, offered next to the book. */
export function learnedRates(lines:readonly LearnedLine[]):PlanningRate[]{
  return lines.map(line=>({code:line.code,description:line.description,type:line.type,unit:line.unit,amount:line.amount,source:line.source,basis:'owner-average-cost'}));
}
