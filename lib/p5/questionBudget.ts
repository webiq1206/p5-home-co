...
interface Question {field:string;label:string;reason:string}
...
export function unaskedQuestions<Q extends Question>(questions:readonly Q[],transcript:readonly AskedEntry[]|undefined,pricedFields:readonly string[]=[]):Q[]{
  ...
  const unique=fresh.filter((question,index)=>!fresh.slice(0,index).some(other=>(!FREE_FORM.has(question.field)&&other.field===question.field)||similar(other.reason,question.reason)));
  const priced=new Set(pricedFields);
  const pricing=unique.filter(question=>priced.has(question.field));
  const others=unique.filter(question=>!priced.has(question.field));
