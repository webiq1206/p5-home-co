...
export function P5Estimator({defaultService='',headingAs='h1'}:{defaultService?:string;headingAs?:'h1'|'h2'}){
  ...
  const projectInput=<>
    <div className={styles.field}><label htmlFor={`${id}-scope`}>Tell us about your project</label><textarea id={`${id}-scope`} rows={4} maxLength={SCOPE_TEXT_LIMIT} value={draft.text} onChange={e=>change({text:e.target.value})} placeholder="For example: Remodel our 8 × 10 ft bathroom. Keep the layout, replace the shower, tile and vanity."/></div>
