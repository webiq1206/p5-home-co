...
## Second round in the same release (found by continuing the live matrix on 24.3)

| Site | Case | Outcome on 24.3 | Fix in 2026-09-25.1 |
| --- | --- | --- | --- |
| Remodeling | One 80 SF tiled shower, plumbing excluded (desktop width) | Priced $14,200 to $15,400 in 205 s with plumbing excluded and two "needed to complete the work but excluded as you asked" dependencies named. Asked a spurious conflict question ("80" or "12") because the reader filed the wall area and the floor area as competing values of one field, and asked the whole-project area for a shower-only job. Customer email carried an audit remark quoting a direct cost ("2 EA @ $2,990") and internal line ids. | `scope.ts` sums stated parts of one quantity that name different surfaces or rooms into one calculated fact (same surface twice, or a stated total, is still asked). `pricing.ts` treats a figure beside an internal id, an "@" or "each" as private cost, and strips line and catalog ids from customer prose without touching identifiers the page relies on; the duplicate-correction note names the work instead of line ids. |
| Handyman | "Only price the trim": 300 LF MDF baseboard and casing for 8 doors | Withheld after 244 s (handed off) because an unpriced protection task, a full-truckload junk line for MDF cutoffs and a "single vs double casing" remark held the range. | Unpriced protection, cleanup and debris tasks on a small job are absorbed into the requested work's labor and disclosed; debris lines join the 15% supporting-work cap. |
| Handyman | Emailed link opened on a new device, then revised | The link exchanged for a device key and showed the saved estimate ($455 to $565); "Update my estimate" reopened it as version 5 with the change kept; the review screen requires the details to be confirmed again before the new version prices (a QA script that skipped the checkbox stalled there, not the product). | No change; see the follow-up run in the production section. |
| Construction | 23-page construction set (12.5 MB, the largest local plan set) | 22 of 23 pages read in about 6.5 minutes (28 drawing-region reads at a median of 25 s each, peak concurrency 12), then held with "some source evidence is still unread" and a Retry control: two drawing regions came back from the reader with `facts` as a JSON string and were rejected as invalid. | `validateExtraction` parses structured fields that arrive as JSON strings before validating them. Reading throughput is recorded here as the measured baseline; scheduling was not changed without a provider-limit test. |
| Handyman (CI) | Resumable 250-page fixture | The repair-only default made the project type a manual answer, which changed the analysis identity on retry and re-read every finished section. | The implied type is supplied as a source-derived fact inside the saved extraction, exactly like the Cabinet intent, so the analysis identity is unchanged; the CI fixture passes again. |
