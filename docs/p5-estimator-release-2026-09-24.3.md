...
9. **PDF range explanation:** remove the outdated claim that the overall range is narrower than the sum of displayed category endpoints. Displayed category amounts are apportioned to reconcile to the overall total.
10. **Cache/release identity:** advance the shared estimator version to 2026-09-24.3. Synchronize the shared engine and hash manifests while preserving all brand-owned adapters, rate books and financial policy.
...
Final local shared suite: 695 passed, 6 skipped, 0 failed (701 total). Each of the four child repositories passed all 104 focused pricing, recovery and synchronization checks. Canonical TypeScript and isolated SQL recovery/persistence checks passed. TypeScript passed in all five repositories. P5 was rechecked with its own clean dependency installation after the shared dependency installation lacked its nodemailer declarations. GitHub build/browser outcomes must be checked on each new commit; they are not claimed complete at this commit. Required checks include the complete shared suite, TypeScript, actual PDF/email projection, persisted audit subset recovery, unchanged-scope reuse, three-attempt timeout exhaustion, unsplittable-output holds, revision race/archive parity and submission recovery. Existing explicit quantity, duplicate-pricing, ownership and omitted-material assertions remain in force.
