...
export async function upsertAccount(r: QboRow): Promise<void> {
  await query(
...
async function syncAccounts(): Promise<number> {
  ...
  for (const r of rows) await upsertAccount(r);
  return rows.length;
}

export async function upsertClass(r: QboRow): Promise<void> {
  await query(
...
async function syncClasses(): Promise<number> {
  ...
  for (const r of rows) await upsertClass(r);
  return rows.length;
}

export async function upsertCustomer(r: QboRow): Promise<void> {
  await query(
...
async function syncCustomers(): Promise<number> {
  ...
  for (const r of rows) await upsertCustomer(r);
  return rows.length;
}

export async function upsertVendor(r: QboRow): Promise<void> {
  await query(
...
async function syncVendors(): Promise<number> {
  ...
  for (const r of rows) await upsertVendor(r);
  return rows.length;
}

export async function upsertTxn(
  entity: (typeof TXN_ENTITIES)[number],
  r: QboRow,
): Promise<void> {
  await query(
...
async function syncTxnEntity(entity: (typeof TXN_ENTITIES)[number]): Promise<number> {
  ...
  for (const r of rows) await upsertTxn(entity, r);
  return rows.length;
}
