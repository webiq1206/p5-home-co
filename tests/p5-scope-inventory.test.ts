...
const scope=()=>({text:'',answers:{},extraction:{takeoffs:[item],reviewNotes:[],documentCoverage:{complete:true,expectedPages:1,pages:[{source:'plans.pdf',page:1,sheet:'A1',revision:'1',status:'read',notes:[]}]}}} as any);
...
test('stale takeoffs cannot bypass inventory when their source is absent or revised',()=>{
  for(const page of [
    {source:'other.pdf',page:1,sheet:'A1',revision:'1',status:'read',notes:[]},
    {source:'plans.pdf',page:1,sheet:'A1',revision:'2',status:'read',notes:[]},
  ]){const input=scope();input.extraction.documentCoverage.pages=[page];assert.equal(retainedScopeInventory(input),null);}
});
