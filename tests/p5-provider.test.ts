...
test('missing page reports are incomplete even when all returned pages say read',async()=>{
 ...
 try{
  ...
  const result=await analyzeBatch('',[{name:'plans.pdf',type:'application/pdf',data:Buffer.from(await pdf.save()),pages:[{source:'plans.pdf',page:1},{source:'plans.pdf',page:2}]}],{},async()=>Response.json({status:'completed', model:'gpt-4.1-2025-04-14',output:[{content:[{type:'output_text',text:JSON.stringify({...extraction,pages:[{source:'plans.pdf',page:1,sheet:'',revision:'',status:'read',notes:[]}]})}]}]}));
...
test('configured OpenAI reads all four scope pages together within its token limit',async()=>{
 ...
 try{
  ...
  const result=await analyzeScope('',[{name:'scope.pdf',type:'application/pdf',data:Buffer.from(await pdf.save())}],{},async(url,options)=>{
   ...
   return Response.json({status:'completed', model:'gpt-4.1-2025-04-14',output:[{content:[{type:'output_text',text:JSON.stringify({...extraction,pages:manifest.map((p:any)=>({...p,sheet:'',revision:'',status:'read',notes:['Blank test sheet.']})),takeoffs:[]})}]}]});
...
test('a provider failure keeps project input private and never substitutes Anthropic',async()=>{
 const before=Object.fromEntries(variables.map(k=>[k,process.env[k]]));for(const k of variables)delete process.env[k];process.env.OPENAI_API_KEY='fixture-only';process.env.ANTHROPIC_API_KEY='fixture-only';
 try{
  const urls:string[]=[];
  await assert.rejects(analyzeScope('Retain the selected cabinet doors',[],{cabinetBaseLf:'20'},async(url,options)=>{
   urls.push(String(url));assert.match(String(options?.body),/cabinetBaseLf/);
   return Response.json({error:{message:'PRIVATE DOCUMENT CONTENT'}},{status:503});
  }),error=>!String(error).includes('PRIVATE DOCUMENT'));
  assert.equal(urls.length,1);assert.ok(urls[0].includes('/responses'));
 }finally{for(const k of variables){if(before[k]===undefined)delete process.env[k];else process.env[k]=before[k];}}
});
