...
test('failed uploads retain a clear retryable error',async()=>{
  ...
  Object.assign(globalThis,{XMLHttpRequest:Transport});
  try{const result=transferProjectFiles(new FormData(),{},()=>{});instance.onerror();await assert.rejects(result,/files are still in this tab/);const rejected=transferProjectFiles(new FormData(),{},()=>{});instance.onload();await assert.rejects(rejected,/Try again/);}finally{Object.assign(globalThis,{XMLHttpRequest:old});}
});
