...
try{
 ...
 assert.equal(results.filter(x=>x.status==='rejected'&&x.reason instanceof PricingPending).length,1,'A concurrent request waits for the saved result');assert.equal(results.filter(x=>x.status==='fulfilled').length,1);assert.equal(provider.calls.filter((s:string)=>s==='MAP').length,2,'Concurrent retries must claim only one provider request');
 const {pricingReplyKey}=await load('pricingWork');assert.equal(pricingReplyKey('MAP',{a:1},false),pricingReplyKey('MAP',{a:1},false));assert.notEqual(pricingReplyKey('MAP',{a:1},false),pricingReplyKey('MAP',{a:1},true),'search and non-search replies are distinct');
