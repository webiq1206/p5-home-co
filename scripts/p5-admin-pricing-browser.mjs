...
import {P5ReferenceCostBudget} from './components/P5ReferenceCostBudget';
const reference={id:'test',source:'Synthetic example only',sourceDate:'2026-09-10',page:1,trade:'Painting',description:'Example painting unit',quantity:100,unit:'SF',unitPrice:100,extendedPrice:10000,priceBasis:'customer-price',commercialStatus:'base',location:'Synthetic',conditions:'Synthetic',warnings:[]};
...
for(const width of [320,390,430,768,1024,1440,1920]){
 ...
 try{
  ...
  const budgets=page.getByRole('region',{name:'Reference unit-cost budgets'});assert.equal(await budgets.count(),1);assert.match(await budgets.innerText(),/20% profit target/);assert.match(await budgets.innerText(),/\$60.00 per SF/);assert.match(await budgets.innerText(),/not observed supplier or payroll costs/);
  assert.equal(await page.getByText("Resolve this item's price basis, scope or unit warnings before deriving a unit-cost budget.",{exact:true}).count(),1);
