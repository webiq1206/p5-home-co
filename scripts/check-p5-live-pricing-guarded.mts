...
async function main(){
 ...
 const directory=resolve('p5-verification','guarded-pricing-mapping-openai-v1');
 const report=await guardedPricingMapping({directory,sourceFingerprint:fingerprint.digest('hex'),request:originalFetch,
  work:async(request:typeof fetch)=>{
   globalThis.fetch=request;
    try{const {runLivePricing}=await import('./check-p5-live-pricing.mts');return await runLivePricing({outputDirectory:directory});}
   finally{globalThis.fetch=originalFetch;}
  }});
 console.log(JSON.stringify({complete:report.complete,error:report.error,provider:report.provider,model:report.model,scenario:report.scenario,elapsedMs:report.elapsedMs,cost:report.cost,productionRouteQualified:report.productionRouteQualified,researchQualified:false}));
