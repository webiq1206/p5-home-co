...
try{
 ...
 globalThis.fetch=async(url:any,options:any)=>{assert.equal(String(url),'https://api.openai.com/v1/responses');const body=JSON.parse(options.body);const manifest=body.input[0].content.filter((v:any)=>v.type==='input_text'&&v.text.startsWith('Source filename:')).flatMap((v:any)=>JSON.parse(v.text.split('Original page manifest: ')[1]));calls.push(body);if(fail)return new Response('fixture outage',{status:503});return Response.json({status:'completed',model:'gpt-4.1-2025-04-14',output:[{content:[{type:'output_text',text:JSON.stringify({summary:'Synthetic bathroom remodel',facts:[{field:'service',value:'bathroom',confidence:.98,source:'typed scope',evidence:'bathroom remodel'},{field:'sqft',value:'80',confidence:.98,source:'plan.pdf',evidence:'80 square feet'},{field:'materials',value:'Porcelain tile',confidence:.97,source:'scope.xlsx',evidence:'Porcelain tile'},{field:'demolition',value:'Remove old tile and vanity',confidence:.97,source:'typed scope',evidence:'Remove old tile and vanity'}],conflicts:[],reviewNotes:[],missingInformation:[],clarifications:[],pages:manifest.map((p:any)=>({...p,sheet:'',revision:'',status:'read',notes:['Blank synthetic sheet.']})),takeoffs:[]})}]}]});};
 ...
 assert.equal(response.status,200);body=await response.json();assert.equal(body.draft.uploads.length,3);assert.equal(body.draft.answers.sqft,'80');assert.equal(body.draft.answers.materials,'Porcelain tile');// The fixture's scope is a bathroom. A brand that offers that service asks nothing more; a brand that does not asks once which of its services applies.
 const offered=((await import(pathToFileURL(path.join(dir,'brand.ts')).href)).ESTIMATOR_BRAND.services as string[]).includes(body.draft.answers.service);
 if(offered)assert.equal(body.questions.length,0,'an offered service needs no project-type question');
 else{assert.equal(body.questions.length,1,'an unoffered service asks for the project type once');assert.equal(body.questions[0].field,'service');}
 assert.equal(calls.length,3);assert.ok(calls.some(c=>JSON.stringify(c).includes('Worksheet: Bathroom')));
 const pdfCall=calls.find(c=>c.input[0].content.some((v:any)=>v.type==='input_file'));
 const sentPdf=pdfCall.input[0].content.find((v:any)=>v.type==='input_file');
 assert.equal((await PDFDocument.load(Buffer.from(sentPdf.file_data.split(',')[1],'base64'))).getPageCount(),2,'adjacent scope pages must reach the model together');
 ...
 globalThis.fetch=originalFetch;delete process.env.OPENAI_API_KEY;await db.database.close();
