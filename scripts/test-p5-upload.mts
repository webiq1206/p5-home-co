...
try{
 ...
 globalThis.fetch=async(url:any,options:any)=>{assert.equal(String(url),'https://api.anthropic.com/v1/messages');const body=JSON.parse(options.body);calls.push(body);if(fail)return new Response('fixture outage',{status:503});return Response.json({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify({summary:'Synthetic bathroom remodel',facts:[{field:'service',value:'bathroom',confidence:.98,source:'typed scope',evidence:'bathroom remodel'},{field:'sqft',value:'80',confidence:.98,source:'plan.pdf',evidence:'80 square feet'},{field:'materials',value:'Porcelain tile',confidence:.97,source:'scope.xlsx',evidence:'Porcelain tile'},{field:'demolition',value:'Remove old tile and vanity',confidence:.97,source:'typed scope',evidence:'Remove old tile and vanity'}],conflicts:[],reviewNotes:[],missingInformation:[],clarifications:[]})}]});};
