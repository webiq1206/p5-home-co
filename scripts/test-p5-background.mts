...
try{
 ...
 const recoveryPayload={input:migrationInput,state:'complete',progress:'Previously completed remotely.',attempts:2,createdAt,retryAt,retryUnits:true,result:{analysis:{modelPolicy:MODEL_POLICY_VERSION,model:ESTIMATOR_MODEL_SNAPSHOT,extraction:{reviewNotes:[]}}}};
 ...
 const ambiguousPayload={...recoveryPayload,input:ambiguousInput,state:'complete',result:{analysis:{modelPolicy:MODEL_POLICY_VERSION,model:ESTIMATOR_MODEL_SNAPSHOT,extraction:{reviewNotes:[]}}}};
 ...
 const unreadInput={...input,text:'Unread source fixture'};reader.failure('unread');
 let unreadJob=await background.queuedJob(unreadInput);
 for(let n=0;n<20&&unreadJob.state!=='failed';n++){await new Promise(r=>setTimeout(r,50));unreadJob=await background.queuedJob(unreadInput);}
 assert.equal(unreadJob.state,'failed');assert.match(unreadJob.progress,/large-plan.pdf: automatic reading could not finish for page 9/);
 assert.equal(reader.calls.filter((call:string)=>call.startsWith('Unread source fixture')).length,1,'exhausted source reads wait for explicit retry');
