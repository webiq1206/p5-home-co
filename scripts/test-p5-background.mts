...
try{
 ...
 await background.queuedJob(interrupted);await new Promise(r=>setTimeout(r,30));
  ...
  const failedVisible=await background.queuedJob(interrupted);assert.equal(failedVisible.state,'failed');assert.match(failedVisible.progress,/Processing paused/);assert.doesNotMatch(failedVisible.progress,/0 of 0/);
  ...
  let recoveredPublic=await requestScope(),sawTypedStatus=recoveredPublic.progress==='Reviewing your typed project scope.';assert.doesNotMatch(recoveredPublic.progress||'',/0 of 0/);retryForm.set('retry','false');
