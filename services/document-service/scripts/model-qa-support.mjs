...
export async function guardedSonnetFetch({file,limitUsd,maxCalls,request=fetch,onRequest=()=>{}}){
 let state;
 ...
 const guarded=async(url,options)=>{
  ...
  state.calls.push(record);await save();onRequest(state.calls.length);
  try{
   const response=await request(url,{...options,redirect:'error'});
   record.httpStatus=response.status;
   const data=await response.clone().json().catch(()=>null);
   if(data?.usage&&Number.isSafeInteger(data.usage.input_tokens)&&Number.isSafeInteger(data.usage.output_tokens)){
   ...
   }else record.status='charge-unknown';
   await save();return response;
  }catch(error){record.status='charge-unknown';await save();throw error;}
 };
 return {request:guarded,summary:()=>({model:state.model,requests:state.calls.length,estimatedUsd:reserved(),estimatedLimitUsd:limitUsd,maxRequests:maxCalls,tokenCountMs:state.tokenCountMs,unknownChargeRequests:state.calls.filter(c=>!c.usage).length,usage:state.calls.map(c=>c.usage).filter(Boolean),note:'Standard-price estimate with guarded reservations, not an invoice or a guaranteed billing cap. Failed calls without usage retain their reserved estimate.'})};
}
