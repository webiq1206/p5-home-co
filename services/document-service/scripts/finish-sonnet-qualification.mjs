...
async function main(){
 ...
 const bundlePath=resolve(process.argv[resumeReserved||resumeReview||resumePlans||resumeOutput?3:2]||'p5-sonnet-fixtures.json');
 ...
 for(const fixture of bundle.fixtures){
  const r=await runFixture(fixture,{root,key:process.env.ANTHROPIC_API_KEY,...(fixture.id==='short'?{seedPages:recovered.pages,recoverLegacyCitationFailure:true,resumeReserved,resumeReview}:{resumePlans,resumeOutput})});
