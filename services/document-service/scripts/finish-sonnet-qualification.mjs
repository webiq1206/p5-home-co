...
async function main(){
 ...
 for(const fixture of bundle.fixtures){
  const r=await runFixture(fixture,{root,key:process.env.ANTHROPIC_API_KEY,...(fixture.id==='short'?{seedPages:recovered.pages,recoverLegacyCitationFailure:true,resumeReserved,resumeReview}:{resumePlans,resumeOutput,resumeCitationOutput,resumeSourceRepair,resumeSourceCorrection,resumeEmptyFact,resumeRepairEvidence,resumeVerifierCitation})});
