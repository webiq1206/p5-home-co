...
async function main(){
 ...
 const resumeReserved=process.argv[2]==='resume-reserved',resumeReview=process.argv[2]==='resume-review',resumePlans=process.argv[2]==='resume-plans-citation',resumeOutput=process.argv[2]==='resume-plans-output',resumeCitationOutput=process.argv[2]==='resume-plans-citation-output',resumeSourceRepair=process.argv[2]==='resume-plans-source-repair';
 const bundlePath=resolve(process.argv[resumeReserved||resumeReview||resumePlans||resumeOutput||resumeCitationOutput||resumeSourceRepair?3:2]||'p5-sonnet-fixtures.json');
 ...
 for(const fixture of bundle.fixtures){
  const r=await runFixture(fixture,{root,key:process.env.ANTHROPIC_API_KEY,...(fixture.id==='short'?{seedPages:recovered.pages,recoverLegacyCitationFailure:true,resumeReserved,resumeReview}:{resumePlans,resumeOutput,resumeCitationOutput,resumeSourceRepair})});
