...
async function main(){
 ...
 const resumeReserved=process.argv[2]==='resume-reserved',resumeReview=process.argv[2]==='resume-review',resumePlans=process.argv[2]==='resume-plans-citation';
 const bundlePath=resolve(process.argv[resumeReserved||resumeReview||resumePlans?3:2]||'p5-sonnet-fixtures.json');
 ...
 for(const fixture of bundle.fixtures){
  const r=await runFixture(fixture,{root,key:process.env.ANTHROPIC_API_KEY,...(fixture.id==='short'?{seedPages:recovered.pages,recoverLegacyCitationFailure:true,resumeReserved,resumeReview}:{resumePlans})});
