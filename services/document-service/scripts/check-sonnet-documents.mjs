...
export function qualificationSpendLimit(id,env=process.env){
 if(id==='short')return 1;
 const raw=env.P5_QA_PLANS_LIMIT_USD;
 if(raw===undefined||raw==='')return 3;
 const value=Number(raw);
 if(!Number.isFinite(value)||value<=0||value>12)throw new ServiceError('qa-plans-spend-limit-invalid',422);
 return value;
}
...
export async function runFixture(fixture,{root,key,parseOnly=false,request=fetch,log=console.log,seedPages=[],recoverLegacyCitationFailure=false,resumeReserved=false,resumeReview=false,resumePlans=false,resumeOutput=false,resumeCitationOutput=false,resumeSourceRepair=false,resumeSourceCorrection=false,resumeEmptyFact=false,resumeRepairEvidence=false,resumeVerifierCitation=false,resumeVerificationBoundary=false,resumePlansReview=false,resumePlansReviewAfterFix=false,resumePlansReviewOutputLimit=false,maxCallsOverride}={}){
 ...
 const config=readConfig({...process.env,DOCUMENT_PROVIDER:'anthropic',OPENAI_API_KEY:key,ANTHROPIC_API_KEY:key,DOCUMENT_MODEL:'claude-sonnet-5',DOCUMENT_VERIFY_MODEL:'claude-sonnet-5',DOCUMENT_DATABASE_URL:'qa-isolated-pglite-not-a-network-database',P5_DOCUMENT_TENANTS_JSON:JSON.stringify({'p5homeco.com':randomBytes(32).toString('hex')})});
 // Historical Sonnet diagnostic only; never used by the estimator worker.
 Object.assign(config,{provider:'anthropic',key,model:'claude-sonnet-5',verifyModel:'claude-sonnet-5'});
 ...
 try{
  ...
  runType=receipt.cached?'resume-or-cache':'new';
   ...
   if(resumePlansReviewAfterFix&&fixture.id==='plans'){
    ...
    recoveryPreflight=false;
    costGuard=await makeCostGuard();document=await store.document('model-qa','source-check',document.id);
    runType='resume-plans-final-review-after-fix';log('plans: preserved all 23 accepted pages, 87 prior calls and both unknown reservations; one post-fix review attempt admitted.');
   }
