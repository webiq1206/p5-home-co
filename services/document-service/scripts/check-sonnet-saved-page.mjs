...
export async function checkSavedPage({root=resolve('.p5-model-qa'),reportPath,key=process.env.ANTHROPIC_API_KEY,env=process.env,request=fetch,log=console.log}={}){
 ...
 const config=readConfig({...env,DOCUMENT_PROVIDER:'anthropic',OPENAI_API_KEY:key,ANTHROPIC_API_KEY:key,DOCUMENT_MODEL:'claude-sonnet-5',DOCUMENT_VERIFY_MODEL:'claude-sonnet-5',DOCUMENT_DATABASE_URL:'qa-no-network-database',P5_DOCUMENT_TENANTS_JSON:JSON.stringify({'p5homeco.com':'synthetic-page-probe-local-configuration-only'})});
