...
test('retries preserve successful page evidence',{skip:!available},async()=>{
 ...
 const before=await store.pages(document.id);await pool.query("UPDATE p5ds_documents SET state='failed',error_code='temporary' WHERE id=$1",[document.id]);await store.retry(tenant,'retry-qa',document.id,'documents');assert.deepEqual((await store.pages(document.id))[0].evidence,before[0].evidence);
});
