...
test('protected activation requires exactly the five authorized tenants',()=>{
 ...
 assert.equal(readConfig({P5_DOCUMENT_REQUIRE_ALL_TENANTS:'true',P5_DOCUMENT_TENANTS_JSON:JSON.stringify(keys),DOCUMENT_DATABASE_URL:'postgres://test',DOCUMENT_MODEL:'test-model',OPENAI_API_KEY:'synthetic'}).tenants['p5homeco.com'],keys['p5homeco.com']);
 assert.throws(()=>readConfig({P5_DOCUMENT_REQUIRE_ALL_TENANTS:'true',P5_DOCUMENT_TENANTS_JSON:JSON.stringify({...keys,'other.example':'synthetic-tenant-key-999999999999999999999'}),DOCUMENT_DATABASE_URL:'postgres://test',DOCUMENT_MODEL:'test-model',OPENAI_API_KEY:'synthetic'}),/incomplete|unauthorized/);
});
