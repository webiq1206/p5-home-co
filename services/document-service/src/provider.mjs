...
export function parseReply(provider,data){
 let text;
...
export class Reader{
 ...
 async call(job,system,input,images,schema,signal,verify=false){
  const c=this.config,estimated=Math.ceil((JSON.stringify(input).length+system.length)/3)+images.reduce((n,i)=>n+Math.ceil(i.bytes.length/256),0)+c.maxOutput;
  ...
  try{
   const built=requestBody(c.provider,verify?c.verifyModel:c.model,system,input,images,schema,c.maxOutput);
