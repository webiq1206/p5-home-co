...
const pool=new pg.Pool({connectionString:config.databaseUrl,max:Math.max(6,config.slots+config.parserSlots),connectionTimeoutMillis:10000,statement_timeout:20000});
...
server.listen(config.port,'0.0.0.0',()=>console.log(JSON.stringify({event:'document-service-start',version:VERSION,port:config.port})));
